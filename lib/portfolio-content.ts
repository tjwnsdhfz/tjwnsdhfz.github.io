...
export const DEFAULT_PORTFOLIO_CONTENT: PortfolioContent = {
  ...
  hero: {
    ...
    primaryCta: { label: "대표 Case Study", href: "https://datumguard-tjwnsdhfz.vercel.app/case-study" },
  ...
  repositoryIntro: {
    eyebrow: "PUBLIC WORK & EVIDENCE",
    title: "주장보다 코드·테스트·변경 기록",
    description:
      "구현 범위, 실패 사례, 테스트 수치와 변경 이력을 공개 저장소에서 다시 확인할 수 있습니다. 메인에는 역할과 결과를 구체적으로 설명할 수 있는 작업만 선별했습니다.",
  ...
  projects: [
    {
      ...
      details: [
        ...
        {
          label: "Transferable approach",
          value: "요구조건 → 산출물 → 독립 재검토 → 변경 기록 → 승인 gate 순서로 검토 흐름을 닫고, 하나라도 실패하면 공식 bundle을 차단합니다.",
        },
        {
  ...
  capabilityIntro: {
    ...
    title: "설계·데이터·협업을 연결하는 실행 역량",
    description: "도구 이름보다 어떤 입력을 다루고, 무엇을 다시 확인하며, 어떤 근거를 남겼는지로 구분했습니다.",
  },
  capabilities: [
    {
...
export function applyGeneralProfile(content: PortfolioContent): PortfolioContent {
  ...
  return {
    ...
    hero: {
      ...
      primaryCta: { ...content.hero.primaryCta, label: defaults.hero.primaryCta.label },
      secondaryCta: { ...content.hero.secondaryCta, label: defaults.hero.secondaryCta.label },
    },
    intro: { ...defaults.intro },
    careerFacts: [
      { ...defaults.careerFacts[0] },
      ...content.careerFacts.slice(1),
    ],
    roleFitIntro: { ...defaults.roleFitIntro },
    roleFit: defaults.roleFit.map((preset, index) => ({
      ...preset,
      evidence: content.roleFit[index]?.evidence || preset.evidence,
    })),
